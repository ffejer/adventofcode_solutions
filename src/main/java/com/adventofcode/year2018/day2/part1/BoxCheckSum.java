...
public class BoxCheckSum extends AbstractProblemSolution {

    public static void main(String[] args) {
        try(Stream<String> stream = Files.lines(Paths.get("./src/main/resources/2018/day/2/input"))) {
            List<Integer> collect = stream.map(BoxCheckSum::calculateChecksumStream).collect(Collectors.toList());
            int checksum2 = 0, checksum3 = 0;
            for(int stringChecksum : collect) {
                if(stringChecksum == 2) checksum2++;
                else if (stringChecksum == 3) checksum3++;
                else if (stringChecksum == 5) { checksum2++; checksum3++; }
            }
    ...
    static int calculateChecksumStream(String input) {
        Map<Character, Integer> frequency = input.chars().boxed().collect(toMap(k -> Character.valueOf((char) k.intValue()),
                v -> 1,
                Integer::sum));
        ...
        int result = 0;
        if(count2 == 1L) result += 2;
        if(count3 == 1L) result += 3;
