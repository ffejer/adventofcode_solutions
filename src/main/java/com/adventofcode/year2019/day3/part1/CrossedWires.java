...
public class CrossedWires extends AbstractProblemSolution {
    ...
    static int solve(String wire1, String wire2) {
        Set<Pair<Integer, Integer>> wire1Coordinates = convertCoordinates(wire1);
        Set<Pair<Integer, Integer>> wire2Coordinates = convertCoordinates(wire2);
        SetUtils.SetView<Pair<Integer, Integer>> intersection = SetUtils.intersection(wire1Coordinates, wire2Coordinates);
        Integer integer = intersection.stream().map(p -> Math.abs(p.getLeft()) + Math.abs(p.getRight())).min(Integer::compareTo).get();
        System.out.println(intersection);

        return integer;
    }

    static Set<Pair<Integer, Integer>> convertCoordinates(String wire) {
        int x = 0;
        int y = 0;
        int dx = 0;
        int dy = 0;
